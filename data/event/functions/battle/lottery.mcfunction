...
# 抽選候補をストレージ保存
execute at 0-0-2-0-2 at @e[tag=Island,tag=!Central,sort=nearest,limit=1] positioned ~ 11 ~ run data modify storage battle:pool Items set from block ~ ~ ~ Items

# 抽選候補の各段の配置数を取得
data modify storage battle:pool Slots set value []
data modify storage battle:pool Slots append from storage battle:pool Items[].Slot
...
function event:battle/count_up
...
function event:battle/get_rank
...
# 当たりをストレージに入れる
function event:battle/get_enemy
