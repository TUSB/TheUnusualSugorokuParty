...
execute unless entity @a[tag=Active,tag=!Leader] run title @a subtitle ["",{"selector":"@a[tag=Active,tag=Leader,limit=1]"},"は島を発見した！！"]
execute if entity @a[tag=Active,tag=!Leader] run title @a subtitle ["",{"selector":"@a[tag=Active,tag=Leader,limit=1]"},"たちは島を発見した！！"]
